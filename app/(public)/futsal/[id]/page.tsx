...
import { TimeSlotGrid } from "@/components/futsal/time-slot-grid"
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const futsalData  = await futsalServices.retrievefutsal(futsalId)
        const slotsData = futsalData?.timeSlots
          ...
          <div className="lg:col-span-1">
            <Card>
              <div className="relative h-64 w-full overflow-hidden rounded-t-lg bg-muted">
                {futsal?.image_url ? (
                  <img
                    src={futsal.image_url || "/placeholder.svg"}
                    alt={futsal.name}
                ...
                {futsal.amenities && futsal.amenities.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {futsal.amenities.map((amenity, index) => (
